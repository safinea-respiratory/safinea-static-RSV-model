...
## ⚠ births_by_month_2seasons.csv is DERIVED

Generated by [`scripts/make_two_seasons.R`](../../scripts/make_two_seasons.R):
the upstream 12 months duplicated forward so the two-season model has births for
both. **Season 2's births are synthetic**, not hub data.

Note the shift differs from the epidemiological files, deliberately:

| File | Shift | Why |
|---|---|---|
| weekly + burden | **+364 days** | Exactly 52 weeks, so every date stays a week-ending Sunday |
| births | **+1 calendar year** | Keeps dates on the first of the month, which `days_in_month()` relies on in `build_dose_table()` |

A 364-day shift would move `2026-09-01` to `2027-08-31` — no longer a month
start, which would break the daily spreading of births. And a calendar-year
shift would break the weekly grid, since 2028 is a leap year. The two files
genuinely need different operations.
